#include "AESCore.h"
#include <cstring>

namespace AESCore {

    static const uint8_t sbox[256] = {
       
    };

    static const uint8_t inv_sbox[256] = {
      
    };

    static const uint32_t Rcon[10] = {
      0x01000000, 0x02000000, 0x04000000, 0x08000000, 0x10000000,
      0x20000000, 0x40000000, 0x80000000, 0x1B000000, 0x36000000
    };

    AES::AES(const uint8_t* key, size_t keyLen) {
        Nb = 4;
        if (keyLen == 16) { Nk = 4; Nr = 10; }
        else if (keyLen == 24) { Nk = 6; Nr = 12; }
        else if (keyLen == 32) { Nk = 8; Nr = 14; }
        else throw std::invalid_argument("Invalid key length. Must be 16, 24, or 32 bytes.");

        KeyExpansion(key);
    }

    AES::~AES() {}

    uint32_t SubWord(uint32_t word) {
        return (sbox[(word >> 24) & 0xFF] << 24) |
               (sbox[(word >> 16) & 0xFF] << 16) |
               (sbox[(word >> 8)  & 0xFF] << 8)  |
               (sbox[word & 0xFF]);
    }

    uint32_t RotWord(uint32_t word) {
        return (word << 8) | (word >> 24);
    }

    void AES::KeyExpansion(const uint8_t* key) {
        roundKeys.resize(Nb * (Nr + 1));
        int i = 0;
        
        while (i < Nk) {
            roundKeys[i] = (key[4*i] << 24) | (key[4*i+1] << 16) | (key[4*i+2] << 8) | key[4*i+3];
            i++;
        }

        i = Nk;
        while (i < Nb * (Nr + 1)) {
            uint32_t temp = roundKeys[i - 1];
            if (i % Nk == 0) {
                temp = SubWord(RotWord(temp)) ^ Rcon[i / Nk - 1];
            } else if (Nk > 6 && (i % Nk == 4)) {
                temp = SubWord(temp);
            }
            roundKeys[i] = roundKeys[i - Nk] ^ temp;
            i++;
        }
    }

    void AES::AddRoundKey(uint8_t state[4][4], int round) const {
        for (int c = 0; c < 4; c++) {
            uint32_t w = roundKeys[round * 4 + c];
            state[0][c] ^= (w >> 24) & 0xFF;
            state[1][c] ^= (w >> 16) & 0xFF;
            state[2][c] ^= (w >> 8) & 0xFF;
            state[3][c] ^= w & 0xFF;
        }
    }

    void AES::SubBytes(uint8_t state[4][4]) const {
        for (int r = 0; r < 4; r++) {
            for (int c = 0; c < 4; c++) {
                state[r][c] = sbox[state[r][c]];
            }
        }
    }

    void AES::ShiftRows(uint8_t state[4][4]) const {
        uint8_t temp;

        // Row 1
        temp = state[1][0];
        state[1][0] = state[1][1];
        state[1][1] = state[1][2];
        state[1][2] = state[1][3];
        state[1][3] = temp;

        // Row 2
        temp = state[2][0];
        state[2][0] = state[2][2];
        state[2][2] = temp;
        temp = state[2][1];
        state[2][1] = state[2][3];
        state[2][3] = temp;

        // Row 3
        temp = state[3][3];
        state[3][3] = state[3][2];
        state[3][2] = state[3][1];
        state[3][1] = state[3][0];
        state[3][0] = temp;
    }

    uint8_t AES::GMul(uint8_t a, uint8_t b) const {
        uint8_t p = 0;
        for (int i = 0; i < 8; i++) {
            if (b & 1) p ^= a;
            uint8_t hi_bit_set = (a & 0x80);
            a <<= 1;
            if (hi_bit_set) a ^= 0x1B;
            b >>= 1;
        }
        return p;
    }

    void AES::MixColumns(uint8_t state[4][4]) const {
        for (int c = 0; c < 4; c++) {
            uint8_t s0 = state[0][c];
            uint8_t s1 = state[1][c];
            uint8_t s2 = state[2][c];
            uint8_t s3 = state[3][c];

            state[0][c] = GMul(0x02, s0) ^ GMul(0x03, s1) ^ s2 ^ s3;
            state[1][c] = s0 ^ GMul(0x02, s1) ^ GMul(0x03, s2) ^ s3;
            state[2][c] = s0 ^ s1 ^ GMul(0x02, s2) ^ GMul(0x03, s3);
            state[3][c] = GMul(0x03, s0) ^ s1 ^ s2 ^ GMul(0x02, s3);
        }
    }

    void AES::EncryptBlock(const uint8_t in[16], uint8_t out[16]) const {
        uint8_t state[4][4];

        // Load input into state (column-major order per FIPS-197)
        for (int r = 0; r < 4; r++) {
            for (int c = 0; c < 4; c++) {
                state[r][c] = in[c * 4 + r];
            }
        }

        AddRoundKey(state, 0);

        for (int round = 1; round < Nr; round++) {
            SubBytes(state);
            ShiftRows(state);
            MixColumns(state);
            AddRoundKey(state, round);
        }

        SubBytes(state);
        ShiftRows(state);
        AddRoundKey(state, Nr);

        // Extract state into output
        for (int r = 0; r < 4; r++) {
            for (int c = 0; c < 4; c++) {
                out[c * 4 + r] = state[r][c];
            }
        }
    }

    void AES::InvSubBytes(uint8_t state[4][4]) const {
        for (int r = 0; r < 4; r++) {
            for (int c = 0; c < 4; c++) {
                state[r][c] = inv_sbox[state[r][c]];
            }
        }
    }

    void AES::InvShiftRows(uint8_t state[4][4]) const {
        uint8_t temp;

        // Row 1
        temp = state[1][3];
        state[1][3] = state[1][2];
        state[1][2] = state[1][1];
        state[1][1] = state[1][0];
        state[1][0] = temp;

        // Row 2
        temp = state[2][0];
        state[2][0] = state[2][2];
        state[2][2] = temp;
        temp = state[2][1];
        state[2][1] = state[2][3];
        state[2][3] = temp;

        // Row 3
        temp = state[3][0];
        state[3][0] = state[3][1];
        state[3][1] = state[3][2];
        state[3][2] = state[3][3];
        state[3][3] = temp;
    }

    void AES::InvMixColumns(uint8_t state[4][4]) const {
        for (int c = 0; c < 4; c++) {
            uint8_t s0 = state[0][c];
            uint8_t s1 = state[1][c];
            uint8_t s2 = state[2][c];
            uint8_t s3 = state[3][c];

            state[0][c] = GMul(0x0e, s0) ^ GMul(0x0b, s1) ^ GMul(0x0d, s2) ^ GMul(0x09, s3);
            state[1][c] = GMul(0x09, s0) ^ GMul(0x0e, s1) ^ GMul(0x0b, s2) ^ GMul(0x0d, s3);
            state[2][c] = GMul(0x0d, s0) ^ GMul(0x09, s1) ^ GMul(0x0e, s2) ^ GMul(0x0b, s3);
            state[3][c] = GMul(0x0b, s0) ^ GMul(0x0d, s1) ^ GMul(0x09, s2) ^ GMul(0x0e, s3);
        }
    }

    void AES::DecryptBlock(const uint8_t in[16], uint8_t out[16]) const {
        uint8_t state[4][4];

        for (int r = 0; r < 4; r++) {
            for (int c = 0; c < 4; c++) {
                state[r][c] = in[c * 4 + r];
            }
        }

        AddRoundKey(state, Nr);

        for (int round = Nr - 1; round > 0; round--) {
            InvShiftRows(state);
            InvSubBytes(state);
            AddRoundKey(state, round);
            InvMixColumns(state);
        }

        InvShiftRows(state);
        InvSubBytes(state);
        AddRoundKey(state, 0);

        for (int r = 0; r < 4; r++) {
            for (int c = 0; c < 4; c++) {
                out[c * 4 + r] = state[r][c];
            }
        }
    }

}

