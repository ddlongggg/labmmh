...
void generate_dummy_file(const std::string &filename, size_t size_mb) {
  std::cout << "Generating dummy file " << filename << " (" << size_mb
            << " MB)...\n";
...
double hash_stream(const std::string &algo, const std::string &filename,
                   size_t file_size_bytes, int ops) {
  ...
  double total_mb =
      static_cast<double>(file_size_bytes * ops) / (1024.0 * 1024.0);
  return total_mb / diff.count(); // MB/s
}

double hash_mmap(const std::string &algo, const std::string &filename,
                 size_t file_size_bytes, int ops) {
  const EVP_MD *md = EVP_get_digestbyname(algo.c_str());
  if (!md)
    throw std::runtime_error("Unknown algorithm: " + algo);
...
void run_benchmark(const std::string &algo, const std::string &filename,
                   size_t file_size_mb, int runs, int ops, std::ofstream &csv) {
  ...
  std::cout << "Stream: " << std::fixed << std::setprecision(2) << std::setw(8)
            << (stream_sum / runs) << " MB/s (avg) | ";
  std::cout << "MMap: " << std::fixed << std::setprecision(2) << std::setw(8)
            << (mmap_sum / runs) << " MB/s (avg)\n";
}
...
int main() {
  ...
  // User requested 100 ops. We'll set ops=100 for 1MB, and ops=10 for 100MB to
  // avoid 2-hour benchmarks, but we can enforce strictly ops=100 if the user
  // absolutely wants to wait. Let's stick strictly to 100 ops for both to
  // satisfy the prompt exactly.
  int ops_1mb = 100;
  int ops_100mb =
      100; // Warning: 30 runs * 100 ops * 100MB = 300GB hashing per algo.

  try {
    for (size_t s : sizes) {
      std::string csv_file =
          "hashing_benchmark_" + std::to_string(s) + "MB.csv";
      ...
      std::cout << "\n--- Benchmark for " << s << " MiB File (" << runs
                << " runs, " << current_ops << " ops) ---\n";
      std::cout << "Warning: Large ops on 100MB file may take several minutes "
                   "per algorithm...\n";
