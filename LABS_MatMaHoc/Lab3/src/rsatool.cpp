...
void PrintUsage() {
    std::cout << "Usage:\n"
              ...
              << "  rsatool encrypt --in <msg.bin> --pub <pub.pem> --out <ct.bin> [--label <optional>] [--format <raw|hex|base64>]\n"
              << "  rsatool decrypt --in <ct.bin> --priv <priv.pem> --out <msg.bin> [--label <optional>] [--format <raw|hex|base64>]\n"
              << "  rsatool benchmark\n";
}
...
int main(int argc, char* argv[]) {
    ...
    try {
        if (command == "keygen") {
            ...
            std::string inFile, pubFile, outFile, label, format = "raw";
            for (size_t i = 0; i < args.size(); ++i) {
            ...
            RSA_Encrypt::EncryptFile(inFile, pubFile, outFile, label, format);
        }
        else if (command == "decrypt") {
            std::string inFile, privFile, outFile, label, format = "raw";
            for (size_t i = 0; i < args.size(); ++i) {
                if (args[i] == "--in" && i + 1 < args.size()) inFile = args[++i];
                else if (args[i] == "--priv" && i + 1 < args.size()) privFile = args[++i];
                else if (args[i] == "--out" && i + 1 < args.size()) outFile = args[++i];
            ...
            RSA_Decrypt::DecryptFile(inFile, privFile, outFile, label, format);
        }
